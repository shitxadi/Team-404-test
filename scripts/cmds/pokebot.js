...
module.exports = {
  config: {
    ...
    longDescription: "A bot that spawns Pokémon at regular intervals.",
  ...
  onStart: async function ({ message, event, threadsData, args }) {
    ...
    // Start Pokémon spawn timer if enabled
    if (args[0] === "on") {
      this.startPokemonTimer(event.threadID, message);
    }
  ...
  startPokemonTimer: function (threadID, message) {
    const interval =  5 * 60 * 1000; // Default is 40 minutes (customize as needed)

    const spawnPokemon = async () => {
      // Check if the bot is enabled for this thread
      const pokebot = await global.GoatBot.threadsData.get(threadID, "settings.pokebot");
      if (!pokebot) return;
  ...
  },
  {
    ...
    setInterval(spawnPokemon, interval);
  },
  ...
  onReply: async function ({ event, Reply, message }) {
    if (event.body.toLowerCase() === Reply.name) {
      ...
      // Restart the spawn timer after a Pokémon is caught
