...
module.exports.onStart = async function ({ api: e, event: a }) {
  // Optional: You can add any initialization code here if needed.
  console.log("YouTube command is ready!");
  return;
};
...
module.exports.handleReply = async function ({
    ...
    try {
        ...
        // Log the response from the download API
        console.log("Download API response:", p);
        ...
        const fileSize = s.statSync(__dirname + "/cache/1.mp4").size;
        console.log("File size:", fileSize);

        // Check if file is too large to send
        if (fileSize > 26e6) {
            e.sendMessage("The file could not be sent because it is larger than 25MB..", a.threadID);
            s.unlinkSync(__dirname + "/cache/1.mp4");
            return;
        }
